...
const SECTIONS = {
  ...
  distribution: {
    title: "Anki Distribution",
    description: "Copy cards from the central Anki instance to separate target AnkiConnect endpoints using temporary .apkg exports.",
    keys: ["DISTRIBUTION_ENDPOINTS", "EXPORT_SHARED_PATH"] as ConfigKey[],
  },
  telegram: {
    title: "Telegram Bot",
...
const KEY_LABELS: Record<ConfigKey, string> = {
  ...
  DISTRIBUTION_ENDPOINTS: "Distribution Endpoints",
  EXPORT_SHARED_PATH: "Export Shared Path",
};
...
const KEY_PLACEHOLDERS: Partial<Record<ConfigKey, string>> = {
  ...
  DISTRIBUTION_ENDPOINTS: "http://anki-child-1:8765, http://anki-child-2:8765",
  EXPORT_SHARED_PATH: "/exports",
};
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Anki Distribution Section */}
      <section className="rounded-lg border border-border p-5 space-y-4">
        <div className="flex items-center gap-2">
          <Share2 className="h-5 w-5 text-violet-500" />
          <div>
            <h2 className="text-lg font-semibold">{SECTIONS.distribution.title}</h2>
            <p className="text-sm text-muted-foreground mt-0.5">
        ...
        <div className="space-y-3">
          {SECTIONS.distribution.keys.map((key) => (
            <SettingsField
              key={key}
              configKey={key}
              status={settings[key]}
        ...
        <p className="text-xs text-muted-foreground">
          The shared path must be mounted at the same path in the portal,
          central Anki instance, and every target Anki instance.
        </p>
      </section>
