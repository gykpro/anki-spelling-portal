...
### 5e. Word-only worksheet extraction
- Upload a worksheet image that contains only a word list (no example sentences)
- Click extract, verify extraction completes
- Verify each extracted word shows with an empty sentence field
- Verify mainSentence and cloze are empty (not garbled)
- Submit to Anki, verify cards created
- Navigate to Enrich with those cards — generate text fields
- Verify sentences are generated by enrichment (fills the gap)

### 5h. PDF upload and extract
- Navigate to `/upload`
- Verify help text says "PNG, JPG, WEBP, or PDF"
- Verify file picker accept includes `.pdf`
- Select or drag a PDF file into the dropzone
- Verify file appears in list with a document icon (not image icon)
- Click extract, verify progress/loading state
- Verify extracted sentences appear (same as image extraction)

### 5i. Mixed image + PDF upload
- Select both an image and a PDF file
- Verify both appear in the file list with correct icons
- Click extract, verify both are processed

### 5f. Edit extracted data
...
### 9g. PDF document via Telegram (manual)
- Send a PDF file as a document to the bot
- Verify bot replies with "Downloading PDF..."
- Verify bot extracts words and processes them
- Verify final result message shows created cards
